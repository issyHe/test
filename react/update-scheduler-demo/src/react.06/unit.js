...
class NativeUnit extends Unit {
    ...
    insertChildAt(parentNode, index, newNode){
        let oldChild = parentNode.children().get(index); // 有没有人在这个位置
        oldChild ? newNode.insertBefore(oldChild) : parentNode.append(newNode);
    }
    patch(diffQueue){
        let deleteChildren = []; //这里放着要删除的节点
        let deleteMap = {}; // 这里放着能复用的节点
